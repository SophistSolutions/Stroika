    ...
    namespace Test2_ZipArchive_ {
        namespace Private_ {
            void    ReadHardwiredZipFile_ ()
            {
                ...
                Assert (sizeof (ksample_zip_) == 2157);
#if     qHasFeature_ZLib && 0
                // Not working yet
                Zip::ArchiveReader  reader (Streams::ExternallyOwnedMemoryInputStream<Byte> (begin (ksample_zip_7z_), end (ksample_zip_7z_)));
                VerifyTestResult ((reader.GetContainedFiles () == Set<String> {L"sample_zip/BlockAllocation-Valgrind.supp", L"sample_zip/Common-Valgrind.supp", L"sample_zip/TODO.txt", L"sample_zip/Tests-Description.txt"}));
