...
NUM_LOCAL_TEST_VALGRINDCONFIGURATIONS=`$LOCAL_VALGRIND_CONFIGS | wc -w`
NUM_REMOTE_TEST_VALGRINDCONFIGURATIONS=`$RASPBERRYPIVALGRINDCONFIGS | wc -w`
echo "***NUM_REMOTE_TEST_VALGRINDCONFIGURATIONS=$NUM_REMOTE_TEST_VALGRINDCONFIGURATIONS"
...
if [ $INCLUDE_LOCAL_TESTS -eq 1 ]; then
	...
	if [ $INCLUDE_VALGRIND_MEMCHECK_TESTS -eq 1 ]; then
		NUM_PASSES_OF_REGTESTS_RUN=$(($NUM_PASSES_OF_REGTESTS_RUN + $LOCAL_VALGRIND_CONFIGS))
	fi
