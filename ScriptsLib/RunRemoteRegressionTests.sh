...
if [ $RUN_IN_DOCKER -eq 0 ]; then
	...
	CMD2Exec+="cd $BUILD_DIR && CONTAINER_NAME=\"$USE_TEST_BASENAME-regtest\" ECHO_DOCKER_COMMANDS=$ECHO_DOCKER_COMMANDS EXTRA_DOCKER_ARGS=\"-e USE_TEST_BASENAME=$USE_TEST_BASENAME -e BUILD_CONFIGURATIONS_MAKEFILE_TARGET=$BUILD_CONFIGURATIONS_MAKEFILE_TARGET -e PRIVATE_COMPILER_BUILDS_DIR=$PRIVATE_COMPILER_BUILDS_DIR\" CMD2RUN=ScriptsLib/RegressionTests.sh ScriptsLib/RunInDockerEnvironment"
fi
