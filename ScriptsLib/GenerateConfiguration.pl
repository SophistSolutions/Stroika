...
my $DEFAULT_CWARNING_FLAGS_SAFE_COMMON_ = $DEFAULT_CWARNING_FLAGS_SAFE_COMMON_ + '-Wno-unused-but-set-variable ';
...
my $DEFAULT_CWARNING_FLAGS_SAFE_COMMON_ = $DEFAULT_CWARNING_FLAGS_SAFE_COMMON_ + '-Wno-unused-local-typedefs ';
...
my $DEFAULT_CWARNING_FLAGS = $DEFAULT_CWARNING_FLAGS_SAFE_COMMON_;
...
my $DEFAULT_CWARNING_FLAGS_CLANG	=	' -Wno-unknown-warning-option ';
...
sub	SetDefaultForCompilerDriver_
{
	...
	if ($CWARNING_FLAGS eq "<<USE_DEFAULTS>>") {
		...
		if ("$^O" eq "aix" and IsGCCOrGPlusPlus_ ($COMPILER_DRIVER)) {
			$CWARNING_FLAGS = $DEFAULT_CWARNING_FLAGS_GCC_AIX;
		}
		elsif (IsClangOrClangPlusPlus_($COMPILER_DRIVER_CPlusPlus)) {
			$CWARNING_FLAGS = $DEFAULT_CWARNING_FLAGS + $DEFAULT_CWARNING_FLAGS_CLANG;
			if ($COMPILER_DRIVER eq "clang++-4.6") {
				$CWARNING_FLAGS = $CWARNING_FLAGS + $DEFAULT_CWARNING_FLAGS_EXTRA4CLANG46;
			}
		}
	}
