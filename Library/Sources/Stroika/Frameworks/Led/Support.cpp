...
*/
void	(*gBeepNotifyCallBackProc) ()	=	nullptr;
...
void	Led_Set_ThrowOSErrException_Handler (void (*throwOSErrExceptionCallback) (OSErr err))
{
	...
	void*	Led_Block_Alloced_sizeof_4_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_8_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_12_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_16_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_20_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_24_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_28_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_32_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_36_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_40_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_44_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_48_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_52_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_56_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_60_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_64_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_68_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_72_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_76_sNextLink	=	nullptr;
	void*	Led_Block_Alloced_sizeof_80_sNextLink	=	nullptr;
#endif
...
void	DumpSupportedInterfaces (IUnknown* obj, const char* objectName, const char* levelPrefix)
{
	...
	try {
		...
		for (DWORD i = 0; ; ++i) {
			...
			if (::RegEnumKeyExA (interfaceKey, i, subKey, &subKeySize, 0, 0, 0, 0) == ERROR_SUCCESS) {
				try {
					...
					if (tmpStr != nullptr)			{	::SysFreeString (tmpStr);		tmpStr = nullptr;	}
					if (SUCCEEDED (obj->QueryInterface (iid, reinterpret_cast<void**> (&test)))) {
						...
						if (iKey != nullptr)			{	::CloseHandle (iKey);			iKey = nullptr;			}
					}
					if (test != nullptr)	{		test->Release ();		test = nullptr; }
				}
				catch (...) {
					OutputDebugStringA (levelPrefix);
		...
		if (test != nullptr)			{	test->Release ();				test = nullptr;			}
		if (tmpStr != nullptr)			{	::SysFreeString (tmpStr);		tmpStr = nullptr;			}
		if (interfaceKey != nullptr)	{	::CloseHandle (interfaceKey);	interfaceKey = nullptr;	}
	}
	catch (...) {
		if (iKey != nullptr)			{	::CloseHandle (iKey);			iKey = nullptr;			}
		if (interfaceKey != nullptr)	{	::CloseHandle (interfaceKey);	interfaceKey = nullptr;	}
