...
/*
 * $Header: /home/lewis/share/RFLWork_CVSRoot/DevRoot/ThirdPartyLibs/Led/LedSrc/Sources/Led_StyledTextIO_HTML.cpp,v 1.3 2009-05-31 15:31:39 Lewis Exp $
 *
 * Changes:
 *	$Log: Led_StyledTextIO_HTML.cpp,v $
 ...
 *	
 *	Revision 2.47  2004/02/10 17:09:10  lewis
 *	cleanup compiler (argsused) warnings
 *	
 *	Revision 2.46  2003/12/12 01:43:24  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.45  2003/05/27 23:10:09  lewis
 ...
 *	
 *	Revision 2.44  2003/05/23 16:41:53  lewis
 *	SPR#1141: got preliminary table reading/writing working for HTML. Doesn't yet
 *	support any attributes to tags - but gets the right number of cells and thier
 *	contents decently written
 *	
 *	Revision 2.43  2003/02/28 19:11:00  lewis
 *	SPR#1316- CRTDBG_MAP_ALLOC/CRTDBG_MAP_ALLOC_NEW Win32/MSVC debug mem leak support
 *	
 *	Revision 2.42  2002/10/31 19:38:31  lewis
 *	SPR#1161- check if strike/sub/sup are on tagstack when writing and dont write extra copy
 *	
 *	Revision 2.41  2002/09/17 02:56:36  lewis
 *	NUL character overwrites end of buffer bug fixed
 *	
 *	Revision 2.40  2002/05/06 21:33:53  lewis
 *	<=============================== Led 3.0.1 Released ==============================>
 *	
 *	Revision 2.39  2002/04/29 15:47:58  lewis
 *	SPR#1081- use IsAsciiSpace() helper instead of isspace() - when working with Led_tChars
 *	
 *	Revision 2.38  2001/11/27 00:29:58  lewis
 *	<=============== Led 3.0 Released ===============>
 *	
 *	Revision 2.37  2001/10/17 20:43:01  lewis
 *	Massive changes - cleaning up doccomments (mostly adding <p>/</p> as appropriate
 *	
 *	Revision 2.36  2001/09/05 23:44:22  lewis
 *	Added GetSinkStream ().EndOfBuffer () calls as part of fix for SPR#1014
 *	
 *	Revision 2.35  2001/08/29 23:36:35  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.34  2001/08/28 18:43:40  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.33  2001/07/31 15:11:34  lewis
 *	fix compiler warnings/quirks MSVC70B2
 *	
 *	Revision 2.32  2001/07/19 02:21:47  lewis
 *	SPR#0960/0961- CW6Pro support, and preliminary Carbon SDK support
 *	
 *	Revision 2.31  2001/07/16 20:07:56  lewis
 *	SPR#0954- support UL/LI lists in reading/writing HTML (using new partial list support in Led)
 *	
 *	Revision 2.30  2001/05/29 21:19:40  lewis
 *	SPR#0942- simple sloppy bug in emitting UNDERLINE tag code fixed
 *	
 *	Revision 2.29  2001/05/29 20:21:42  lewis
 ...
 *	
 *	Revision 2.28  2001/05/10 21:55:22  lewis
 *	SPR#0913- small bug with outputting HTML nested font info - based on recent changes
 *	
 *	Revision 2.27  2001/05/09 19:49:18  lewis
 *	SPR#0912- if comma in HTML font name - ignore successive ones, and just take the first
 *	
 *	Revision 2.26  2001/05/09 19:33:41  lewis
 *	SPR#0910- MANY big changes to HTML writer - maintaining a stack of tags. Point its to
 *	get right nesting semantics required for XML. Also - bugs with the current HTML reader
 *	require those nesting semantics anyhow. Also - fixed color names to be lowercase. And -
 *	fixed to write out some colors by name (same as ones we recognize on reading).
 *	
 *	Revision 2.25  2001/05/08 23:16:56  lewis
 *	SPR#0909- All HTML tags I generate now are lowercase (rather than some random
 *	mishmosh choice of case)
 *	
 *	Revision 2.24  2001/05/08 23:09:46  lewis
 *	SPR#0908- be more XML-friendly - surround size= etc args with quotes
 *	
 *	Revision 2.23  2001/05/01 14:15:59  lewis
 *	include <cstdio>
 *	
 *	Revision 2.22  2001/02/05 22:28:52  lewis
 *	as part of SPR#0853- ALSO fix <br> / softline break support
 *	
 *	Revision 2.21  2001/02/05 17:18:42  lewis
 *	SPR#0853. Original request was to support align=center/etc in <p> tags. I DID fix this.
 *	But also had to substantially rewrite the emitting of NLs and the reading of <p> codes.
 *	Now these work much better - I believe (more true to the HTML spec). AND they support justification.
 *	
 *	Revision 2.20  2001/01/20 01:14:30  lewis
 *	cleanups. SPR#0852- fixup so works a bit better on Non-UNICODE mac
 *	
 *	Revision 2.19  2001/01/19 22:44:22  lewis
 *	SPR#0852- improve UNICODE HTML support. Handle entity refs properly - both named
 *	and numeric. Still must test/fixup a bit on mac maybe?
 *	
 *	Revision 2.18  2001/01/03 15:56:47  Lewis
 *	fix a couple small problems compiling iwth the borlaand C++ compiler - including
 *	qStaticInitializerOfPairOfStringStringInternalCompilerBug
 *	
 *	Revision 2.17  2001/01/03 15:56:05  Lewis
 *	fix a couple small problems compiling iwth the borlaand C++ compiler - including
 *	
 *	Revision 2.16  2000/09/26 15:20:37  lewis
 *	SPR#0838- Added gotStartQuote to StyledTextIOReader_HTML::ParseHTMLTagArgOut ()
 *	to make it better handle embedded quotes
 *	
 *	Revision 2.15  2000/09/26 13:55:05  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.14  2000/09/20 09:02:39  lewis
 *	silcence a few warnings with qSilenceAnnoyingCompilerWarnings
 *	
 *	Revision 2.13  2000/08/29 15:10:43  lewis
 *	SPR#0819- support HTML writer writing out AHREF= tags for
 *	StandardURLStyleMarker classes
 *	
 *	Revision 2.12  2000/08/29 14:55:51  lewis
 *	try to do a better job closing of HTML font/bold etc tags - but still not
 *	completely correct. Need total rewrite to do correctly
 *	
 *	Revision 2.11  2000/08/29 13:54:18  lewis
 *	Related to SPR#0820 - add kColorNameTable support so we can read color NAMES
 *	in HTML text
 *	
 *	Revision 2.10  2000/08/29 13:35:38  lewis
 *	SPR#0819- handle reading '<a href=foo>bar</a>' properly - creating a URL
 *	object with that 'bar' text as the enclosed text.
 *	
 *	Revision 2.9  2000/08/28 22:54:24  lewis
 *	SPR#0820- fix a few HTML </font></b> tag problems. This code is still fragile crap -
 *	but now works for a few more cases...
 *	
 *	Revision 2.8  2000/06/23 21:39:57  lewis
 *	SPR#0806- support reading NUL-bytes in input text
 *	
 *	Revision 2.7  2000/05/01 23:41:51  lewis
 *	SPR#0622- fix reading/writing of FONT/COLOR info
 *	
 *	Revision 2.6  2000/04/16 14:39:38  lewis
 *	trivial tweeks to get compiling on Mac
 *	
 *	Revision 2.5  2000/04/14 22:40:27  lewis
 *	SPR#0740- namespace support
 *	
 *	Revision 2.4  2000/02/02 17:14:03  lewis
 *	SPR#0707- add (primitive) HTML <DIV align=> support
 *	
 *	Revision 2.3  1999/12/21 03:15:27  lewis
...
{
	if (start and fSaveHTMLInfoInto != NULL) {
...
{
	if (fGetHTMLInfoFrom != NULL and fGetHTMLInfoFrom->fDocTypeTag.length () != 0) {
		write (fGetHTMLInfoFrom->fDocTypeTag);
	}

	if (fGetHTMLInfoFrom == NULL or fGetHTMLInfoFrom->fHTMLTag.length () == 0) {
	...
	if (fGetHTMLInfoFrom == NULL or fGetHTMLInfoFrom->fHeadTag.length () == 0) {
...
void	StyledTextIOWriter_HTML::WriteBody (WriterContext& writerContext)
{
	...
	if (fGetHTMLInfoFrom == NULL or fGetHTMLInfoFrom->fStartBodyTag.length () == 0) {
