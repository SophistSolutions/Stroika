...
String ProviderConfiguration::ToString () const
{
    ...
    if (openid_configuration_uri) {
        sb << ", openidConfigurationURI: " << *openid_configuration_uri;
    }
    if (auth_uri) {
        sb << ", authURI: " << *auth_uri;
    }
    if (token_uri) {
        sb << ", tokenURI: " << *token_uri;
    }
    if (userinfo_endpoint) {
        sb << ", userinfoEndpoint: " << *userinfo_endpoint;
    }
    if (revocation_endpoint) {
        sb << ", revocationEndpoint: " << *revocation_endpoint;
    }
    if (auth_provider_x509_cert_url) {
        sb << ", authProviderX509CertURL: " << *auth_provider_x509_cert_url;
    }
    if (introspection_endpoint) {
        sb << ", introspectionEndpoint: " << *introspection_endpoint;
    }
    if (tokeninfo_endpoint) {
        sb << ", tokeninfoEndpoint: " << *tokeninfo_endpoint;
    }
    sb << "}"sv;
    return sb;
}
