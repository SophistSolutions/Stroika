...
String SystemConfiguration::Memory::ToString () const
{
    StringBuilder sb;
    sb << "{"sv;
    sb << "Page-Size: "sv << fPageSize;
    sb << ", Total-Physical-RAM: "sv << fTotalPhysicalRAM;
...
#elif qStroika_Foundation_Common_Platform_POSIX
        {
             ...
             */
                ...
                for (const String& line :
                     BinaryToText::Reader::New (IO::FileSystem::FileInputStream::New (kProcUptimeFileName_, IO::FileSystem::FileInputStream::eNotSeekable))
                         .ReadLines ()) {
...
SystemConfiguration::CPU Common::GetSystemConfiguration_CPU ()
{
...
#if qStroika_Foundation_Common_Platform_Linux
    {
        ...
        for (const String& line :
             BinaryToText::Reader::New (IO::FileSystem::FileInputStream::New (kProcCPUInfoFileName_, IO::FileSystem::FileInputStream::eNotSeekable))
                 .ReadLines ()) {
...
SystemConfiguration::OperatingSystem Common::GetSystemConfiguration_ActualOperatingSystem ()
{
    ...
    static const OperatingSystem kCachedResult_ = [] () -> OperatingSystem {
        ...
        if (tmp.fShortPrettyName.empty ()) {
            try {
                String n = Streams::BinaryToText::Reader::New (IO::FileSystem::FileInputStream::New ("/etc/centos-release"sv)).ReadAll ().Trim ();
        ...
        if (tmp.fShortPrettyName.empty ()) {
            try {
                String n = Streams::BinaryToText::Reader::New (IO::FileSystem::FileInputStream::New ("/etc/redhat-release"sv)).ReadAll ().Trim ();
