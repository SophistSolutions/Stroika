	...
	wstring	mkCanBeEmptyReason_ (Status status, const wstring& reason)
		{
			...
			if (effectiveReason.empty ()) {
				switch (status) {
					...
					case	StatusCodes::kNotFound:				effectiveReason = L"URL not found"; break;
					case	405:								effectiveReason = L"Method Nnt allowed"; break;
					case	410:								effectiveReason = L"Gone (service has been discontinued)"; break;
					case	413:								effectiveReason = L"Request entity too large"; break;
					case	415:								effectiveReason = L"Unsupported media type"; break;
					case	StatusCodes::kServiceUnavailable:	effectiveReason = L"Service temporarily unavailable: try again later"; break;
				}
			}
