...
URI URI::Parse (const String& rawURL)
{
    ...
    if (rawURL.Matches (kParseURLRegExp_, nullptr, &scheme, nullptr, &authority, &path, nullptr, &query, nullptr, &fragment)) {
        ...
        static const Execution::Exception<runtime_error> kException_{"Ill-formed URI"sv};
...
URI URI::ParseRelative (const String& rawRelativeURL)
{
    ...
    if (rawRelativeURL.Matches (kParseRelativeURLRegExp_, &path, nullptr, &query, nullptr, &fragment)) {
        ...
        static const Execution::Exception<runtime_error> kException_{"Ill-formed relative URI"sv};
        Execution::Throw (kException_); // doesn't match regexp in https://tools.ietf.org/html/rfc3986#appendix-B
    }
}
...
String URI::AsString_ (optional<StringPCTEncodedFlag> pctEncode) const
{
    ...
    if (fAuthority_ and not(fPath_.empty () or fPath_.StartsWith ("/"sv))) {
        ...
        static const Execution::Exception<runtime_error> kException_{"This is not a legal URI to encode (authority present, but path not empty or absolute)"sv};
...
void URI::CheckValidPathForAuthority_ (const optional<Authority>& authority, const String& path)
{
    ...
    if (authority and (not path.empty () and not path.StartsWith ("/"sv))) {
        static const Execution::Exception<runtime_error> kException_{"A URI with an authority must have an empty path, or an absolute path"sv};
        Execution::Throw (kException_);
    }
}
...
URI URI::Combine (const URI& overridingURI) const
{
    ...
    if (not baseURI.GetScheme ()) {
        static const Execution::Exception<runtime_error> kException_{"Scheme is required in base URI to combine with another URI"sv};
        Execution::Throw (kException_);
    }
...
strong_ordering URI::TWC_ (const URI& lhs, const URI& rhs)
{
    ...
    if (auto cmp = Common::StdCompat::compare_three_way{}(lhs.GetScheme (), rhs.GetScheme ()); cmp != strong_ordering::equal) {
        return cmp;
    }
    if (auto cmp = Common::StdCompat::compare_three_way{}(lhs.GetAuthority (), rhs.GetAuthority ()); cmp != strong_ordering::equal) {
        return cmp;
    }
    if (auto cmp = Common::StdCompat::compare_three_way{}(lhs.GetPath (), rhs.GetPath ()); cmp != strong_ordering::equal) {
        return cmp;
    }
    if (auto cmp = Common::StdCompat::compare_three_way{}(lhs.GetQuery (), rhs.GetQuery ()); cmp != strong_ordering::equal) {
        return cmp;
    }
    if (auto cmp = Common::StdCompat::compare_three_way{}(lhs.GetFragment (), rhs.GetFragment ()); cmp != strong_ordering::equal) {
        return cmp;
    }
