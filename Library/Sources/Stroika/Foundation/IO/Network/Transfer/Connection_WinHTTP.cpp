...
namespace {
    ...
    public:
        ...
        virtual Response Send (const Request& request) override
        {
            ...
            // See https://github.com/SophistSolutions/Stroika/issues/576 (STK-442) - we pre-set to avoid double try on failure, but
            ...
            if (not fOptions_.fReturnSSLInfo and
                not fOptions_.fFailConnectionIfSSLCertificateInvalid.value_or (kDefault_FailConnectionIfSSLCertificateInvalid)) {
        ...
        RetryWithNoCERTCheck:
            ...
            //      TODO!!!
            //
            // See https://github.com/SophistSolutions/Stroika/issues/576 (STK-442)
            //
        ...
        RetryWithAuth:
            ...
            if (useSecureHTTP and fOptions_.fReturnSSLInfo) {
                ...
                resultSSLInfo.fValidationStatus = sslExceptionProblem ? Response::SSLResultInfo::ValidationStatus::eSSLFailure
    ...
    private:
        static wstring Extract_WinHttpHeader_ (HINTERNET hRequest, DWORD dwInfoLevel, LPCWSTR pwszName, LPDWORD lpdwIndex)
        {
            DWORD size = 0;
