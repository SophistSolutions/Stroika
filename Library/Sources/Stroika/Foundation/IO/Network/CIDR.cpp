    ...
    CIDR read_ (const String& cidrNotation, InternetAddress::AddressFamily addressFamily)
    {
        if (auto i = cidrNotation.RFind ('/')) {
            ...
            if (not ia.GetAddressSize ().has_value ()) [[unlikely]] {
                static const Execution::Exception<runtime_error> k_ =
                    Execution::Exception<runtime_error>{"CIDR format exception: cannot use CIDR notation with that type of internet address"sv};
                Execution::Throw (k_);
            }
            if (*ia.GetAddressSize () * 8 < nBits) [[unlikely]] {
                static const Execution::Exception<runtime_error> k_ =
                    Execution::Exception<runtime_error>{"CIDR format exception: number of significant bits too large"sv};
                Execution::Throw (k_);
            }
            return CIDR{ia, nBits};
        }
        static const Execution::Exception<runtime_error> k_ =
            Execution::Exception<runtime_error>{"CIDR format exception: doesn't contain a / character"sv};
        Execution::Throw (k_);
    }
}
