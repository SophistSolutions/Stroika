...
namespace Stroika::Foundation::Containers {
    ...
    {
        return items.All ([&] (ArgByValueType<T> i) { return this->Contains (i); });
    }
    template <typename T>
    bool Set<T>::ContainsAny (const Iterable<value_type>& items) const
    {
        return items.Any ([&] (ArgByValueType<T> i) { return this->Contains (i); });
    }
    template <typename T>
    ...
    bool Set<T>::_IRep::_Equals_Reference_Implementation (const typename Iterable<value_type>::_IRep& rhs) const
    {
        ...
        for (auto i = rhs.MakeIterator (); not i.AtEnd (); ++i) {
