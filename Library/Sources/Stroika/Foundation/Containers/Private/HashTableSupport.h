...
namespace Stroika::Foundation::Containers::Private {
     ...
     * 
     *  \note bool USING_IREP:
     *      Can we just peek from HashTableBasedContainer<> into the fData_ and do the array operations? Yes, if Rep_ is not
     *      type-erased, but no otherwise. If we specify false, we can avoid putting references to the HashTableBasedContainerIRep
    ...
    public:
        ...
        HashTableBasedContainer (ARGS... args);
    ...
    /**
     *  \brief impl detail for array based container support (see HashTableBasedContainer docs on bool USING_IREP)
     */
    ...
    public:
         ...
         */
        template <typename... ARGS>
        HashTableBasedContainerRepImpl (ARGS... args);
