...
namespace {
    struct MyZipLibInStream_ final : zlib_filefunc64_def {
        InputStream::Ptr<byte> fInStream_;
#if qStroika_Foundation_Debug_AssertionsChecked
        bool fOpened_{false};
#endif
        MyZipLibInStream_ (const InputStream::Ptr<byte>& in)
            : fInStream_{in}
        {
            ...
            this->zseek64_file = [] (voidpf opaqueStream, [[maybe_unused]] voidpf stream, ZPOS64_T offset, int origin) -> long {
                ...
                switch (origin) {
                    ...
                    case ZLIB_FILEFUNC_SEEK_CUR:
                        myThis->fInStream_.Seek (eFromCurrent, offset);
                        break;
                    case ZLIB_FILEFUNC_SEEK_END:
                        myThis->fInStream_.Seek (eFromEnd, offset);
                        break;
                    default:
                        AssertNotReached ();
...
namespace {
    ...
    struct Rep_ final : public Reader::IRep {
        MyZipLibInStream_ fInSeekStream_;
        unzFile           fZipFile_;

        Rep_ (const InputStream::Ptr<byte>& in)
            : fInSeekStream_{in}
        ...
        virtual Set<String> GetContainedFiles () const override
        {
            ...
            ThrowIfMinizipErr_ (unzGetGlobalInfo64 (fZipFile_, &gi), "unzGetGlobalInfo64"sv);
            for (size_t i = 0; i < gi.number_entry; i++) {
                ...
                //char charCrypt = ' ';
                ThrowIfMinizipErr_ (::unzGetCurrentFileInfo64 (fZipFile_, &file_info, filename_inzip, sizeof (filename_inzip), NULL, 0, NULL, 0),
                                    "unzGetGlobalInfo64"sv);
                if ((i + 1) < gi.number_entry) {
                    ThrowIfMinizipErr_ (::unzGoToNextFile_ (fZipFile_), "unzGoToNextFile_"sv);
                }
                //tmphack
