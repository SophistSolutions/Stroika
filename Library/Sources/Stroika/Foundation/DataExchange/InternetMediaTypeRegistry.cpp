...
namespace {
    Bijection<InternetMediaType, FileSuffixType> ReadGlobsFile_ ()
    {
        ...
        for (Sequence<String> line : DataExchange::Variant::CharacterDelimitedLines::Reader{{':'}}.ReadMatrix (IO::FileSystem::FileInputStream::mk (L"/usr/share/mime/globs"))) {
            if (line.length () == 2) {
                String glob = line[1];
                if (glob.StartsWith ('*')) {
...
namespace {
    ...
    struct OptionsFileHelper_ {
        ...
        static HKEY OpenWithCreateAlongPath (HKEY parentKey, const String& path, REGSAM samDesired = KEY_READ | KEY_WRITE)
        {
            ...
            Debug::TraceContextBumper trcCtx (Characters::Format (L"OptionsFileHelper::OpenWithCreateAlongPath (0x%x, %s)", reinterpret_cast<int> (parentKey), path.c_str ()).c_str ());
#endif
