...
// Issue is tmp variable convert from float to int, somehow goes negative... Not sure my bug or MSFT - must look more carefully
                ...
                if (dv <= dMin) {
                    return time_point<Clock, Duration> (duration_cast<Duration> (std::chrono::duration<DurationSecondsType>{dMin}));
                }
                if (dv >= dMax) {
                    return time_point<Clock, Duration> (duration_cast<Duration> (std::chrono::duration<DurationSecondsType>{dMax}));
                }
