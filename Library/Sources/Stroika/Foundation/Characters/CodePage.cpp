...
namespace {
    ...
    struct codecvt_iso10646_ : std::codecvt<wchar_t, char, mbstate_t> {
        virtual result do_in (mbstate_t& _State, const char* _First1, const char* _Last1, const char*& _Mid1, wchar_t* _First2,
                              wchar_t* _Last2, wchar_t*& _Mid2) const
        {
        ...
        virtual result do_out (mbstate_t& _State, const wchar_t* _First1, const wchar_t* _Last1, const wchar_t*& _Mid1, char* _First2,
                               char* _Last2, char*& _Mid2) const
        {
...
namespace Stroika::Foundation::Characters {
    ...
    const codecvt<wchar_t, char, mbstate_t>& LookupCodeConverter (const String& charset)
    {
        ...
        if (String::EqualsComparer{CompareOptions::eCaseInsensitive}(charset, "utf-8")) {
            static const codecvt_utf8<wchar_t> kConverter_; // safe to keep static because only read-only const methods used
            return kConverter_;
        }
        else if (String::EqualsComparer{CompareOptions::eCaseInsensitive}(charset, "ISO-8859-1")) {
