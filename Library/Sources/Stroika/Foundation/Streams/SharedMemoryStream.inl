...
namespace Stroika::Foundation::Streams::SharedMemoryStream {
    ...
    namespace Private_ {
        ...
        private:
            ...
            qStroika_ATTRIBUTE_NO_UNIQUE_ADDRESS_TRY_ANYHOW conditional_t<kLocking_, Execution::WaitableEvent, Common::Empty> fMoreDataWaiter_{}; // not a race cuz always set/reset when holding fMutex; no need to pre-set cuz auto set when someone adds data (Write)
        ...
        private:
            ...
            nonvirtual void FreeUpSpaceIfNeeded_ ()
            {
                ...
                if (elts2Reclaim * sizeof (ELEMENT_TYPE) >= kMinData2Reclaim_ and IsOpenRead () and IsOpenWrite ()) [[unlikely]] {
                    ...
                    fReadCursor_  = fData_.begin () + static_cast<size_t> (readOffset - fSpaceClearedFromStreamHead_);
                    fWriteCursor_ = fData_.begin () + static_cast<size_t> (writeOffset - fSpaceClearedFromStreamHead_);
        ...
        private:
            qStroika_ATTRIBUTE_NO_UNIQUE_ADDRESS_TRY_ANYHOW mutable LOCK_IMPL fMutex_;
