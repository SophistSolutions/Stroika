...
const availableProviderOptions = computed(() =>
    availableProviders.value?.map((i) => {
        return { label: capitalize_(i.provider), value: i.provider };
    })
);
const activeProvider: Ref<IOAuthProviderConfig | undefined> = auth.activeProvider;

const loginWith: Ref<QSelectOption> = ref(
    availableProviderOptions.value?.length
        ? availableProviderOptions.value[0]
...
async function login() {
    const useProvider: IOAuthProviderConfig | undefined = availableProviders.value?.find(
        (i) => {
            if (loginWith.value.value == i.provider) {
                return i;
            }
        }
    );
    ...
    <q-page padding>
        ...
        <div class="justify-center row">
            <q-card class="col-11 pageCard">
                <q-card-section>
                    <div class="row">
                        Configuration (availableProviders - from Backend webservices and its
                        configuration):
                    </div>
    ...
    "Auth" : {
        "OAuth2" : [
            ...
            <q-card class="col-11 pageCard" v-if="activeProvider">
                <q-card-section>
                    <div class="row">
                        Configuration (activeProvider - if logged in, will be valid, but can be valid
                        even if not logged in):
                    </div>
                    <div class="row">
                        <pre v-if="activeProvider">{{ JSON.stringify(activeProvider, null, 2) }}</pre>
                    </div>
                </q-card-section>
            </q-card>
                ...
                <q-card-section>
                    ...
                    <div class="q-gutter-y-sm">
                        ...
                        <div class="row" v-if="auth.authorizationTokens.value?.expiresAt">
                            ...
                            <div class="col">
                                {{ auth.authorizationTokens.value.expiresAt }} ({{ expiresIn }})
                            </div>
                        </div>
                        ...
                        <div class="row" v-if="!isAuthenticated">NOT CURRENTLY LOGGED IN.</div>
                        <div class="row">
                            <div class="col-1"></div>
