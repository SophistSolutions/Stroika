...
namespace {
    void    ReadHardwired7zFile_ ()
    {
        ...
        Assert ((reader.GetContainedFiles () == Set<String> {L"sample_zip/BlockAllocation-Valgrind.supp", L"sample_zip/Common-Valgrind.supp", L"sample_zip/TODO.txt", L"sample_zip/Tests-Description.txt"}));

        {
            using   Streams::InputStream;
            using   Streams::TextReader;
            using   Memory::Byte;
            Assert (reader.GetData (L"sample_zip/TODO.txt").size () == 243);
            Assert (reader.GetData (L"sample_zip/BlockAllocation-Valgrind.supp").size () == 4296);
            Assert (reader.GetData (L"sample_zip/Common-Valgrind.supp").size () == 1661);
            Assert (reader.GetData (L"sample_zip/Tests-Description.txt").size () == 1934);
            Assert (TextReader (reader.GetData (L"sample_zip/TODO.txt").As<InputStream<Byte>> ()).ReadAll ().Contains (L"Once any of the ThreadSafetyBuiltinObject tests work - with the locking stuff - add more concrete tyeps"));
            Assert (TextReader (reader.GetData (L"sample_zip/Tests-Description.txt").As<InputStream<Byte>> ()).ReadAll ().Contains (L"[30]	Foundation::DataExchange::Other"));
            try {
