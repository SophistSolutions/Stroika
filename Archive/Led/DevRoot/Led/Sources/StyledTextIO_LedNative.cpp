...
/*
 * $Header: /cygdrive/k/CVSRoot/Led/Sources/StyledTextIO_LedNative.cpp,v 2.11 2003/02/28 19:11:00 lewis Exp $
 *
 * Changes:
 *	$Log: StyledTextIO_LedNative.cpp,v $
 *	Revision 2.11  2003/02/28 19:11:00  lewis
 *	SPR#1316- CRTDBG_MAP_ALLOC/CRTDBG_MAP_ALLOC_NEW Win32/MSVC debug mem leak support
 *	
 *	Revision 2.10  2003/02/24 21:56:59  lewis
 ...
 *	*** empty log message ***
 *	
 *	Revision 2.5  2000/04/16 14:39:38  lewis
 *	trivial tweeks to get compiling on Mac
 *	
 *	Revision 2.4  2000/04/14 22:40:27  lewis
 *	SPR#0740- namespace support
 *	
 *	Revision 2.3  2000/03/13 17:36:35  lewis
 *	Support reading/writing with qWideCharacters turned on
 *	
 *	Revision 2.2  1999/12/18 04:13:19  lewis
 *	*** empty log message ***
 *	
...
#include	"StyledTextIO_LedNative.h"
	...
	inline	void	_DO_ALIGN_ASSERTS_Version4_ ()
		{
			Led_Assert (sizeof (PortableStyleRunData_Version4) == 268);
			Led_Assert (offsetof (PortableStyleRunData_Version4, fItalic) == 256);
			Led_Assert (offsetof (PortableStyleRunData_Version4, fBoldWeight) == 257);
			Led_Assert (offsetof (PortableStyleRunData_Version4, fUnderline) == 258);
			Led_Assert (offsetof (PortableStyleRunData_Version4, fFontSize) == 260);
			Led_Assert (offsetof (PortableStyleRunData_Version4, fLength) == 264);
		}
	...
	inline	void	_DO_ALIGN_ASSERTS_Version5_ ()
		{
			Led_Assert (offsetof (PortableStyleRunData_Version5, fThisRecordLength) == 0);
			Led_Assert (offsetof (PortableStyleRunData_Version5, fStyleSet) == 1);
			Led_Assert (offsetof (PortableStyleRunData_Version5, fPointSize) == 2);
			Led_Assert (offsetof (PortableStyleRunData_Version5, fLength) == 4);
			Led_Assert (offsetof (PortableStyleRunData_Version5, fFontName) == 8);
			//Led_Assert (offsetof (PortableStyleRunData_Version5, fFontName) == 16);
			//Led_Assert (sizeof (PortableStyleRunData_Version5) == 16+256);
			Led_Assert (sizeof (PortableStyleRunData_Version5) == 8+256);
		}
	...
	inline	void	_DO_ALIGN_ASSERTS_Version6_ ()
		{
			Led_Assert (offsetof (PortableStyleRunData_Version6, fThisRecordLength) == 0);
			Led_Assert (offsetof (PortableStyleRunData_Version6, fStyleSet) == 2);
			Led_Assert (offsetof (PortableStyleRunData_Version6, fPointSize) == 4);
			Led_Assert (offsetof (PortableStyleRunData_Version6, fColor) == 6);
			Led_Assert (offsetof (PortableStyleRunData_Version6, fLength) == 12);
			Led_Assert (offsetof (PortableStyleRunData_Version6, fFontName) == 16);
			Led_Assert (sizeof (PortableStyleRunData_Version6) == 16+256);
		}
...
void	StyledTextIOReader_LedNativeFileFormat::Read_Version4 (const char* cookie)
{
	...
	{
		Led_SmallStackBuffer<char>	buf (totalTextLength);
		if (GetSrcStream ().read (buf, totalTextLength) != totalTextLength) {
			Led_ThrowBadFormatDataException ();
		}
		#if		qWideCharacters
			size_t nChars	= totalTextLength;
			Led_SmallStackBuffer<Led_tChar>	unicodeText (nChars);
#if 1
	...
	// Read Style Runs
	{
		...
		{
			Led_SmallStackBuffer<PortableStyleRunData_Version4>	portableStyleRuns (nStyleRuns);
...
void	StyledTextIOReader_LedNativeFileFormat::Read_Version5 (const char* cookie)
{
	...
	// Read Style Runs
	{
		...
		Led_SmallStackBuffer<char>	portableStyleRunsBuffer (howManyBytesOfStyleInfo);
...
SimpleEmbeddedObjectStyleMarker*	StyledTextIOReader_LedNativeFileFormat::InternalizeEmbedding (Led_PrivateEmbeddingTag tag, size_t howManyBytes)
{
	...
	Led_SmallStackBuffer<char>	dataBuf (howManyBytes);
...
void	StyledTextIOWriter_LedNativeFileFormat::Write_Version5 ()
{
...

	{
		...
		Led_Assert (checkTotalWritten == totalTextLength);
	}
...
void	StyledTextIOWriter_LedNativeFileFormat::Write_Version6 ()
{
	...
	{
		// write a length-of-text count, and then the text
		Led_SmallStackBuffer<Led_tChar>	buf (totalTextLength);
