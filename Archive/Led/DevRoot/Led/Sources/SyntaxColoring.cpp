...
/*
 * $Header: /cygdrive/k/CVSRoot/Led/Sources/SyntaxColoring.cpp,v 2.16 2003/01/17 01:02:16 lewis Exp $
 *
 * Description:
 *
 *
 ...
 *
 * Changes:
 *	$Log: SyntaxColoring.cpp,v $
 *	Revision 2.16  2003/01/17 01:02:16  lewis
 *	SPR#1244- fix buf where from could be > to, causing to-from compiutation to be negative (very large unsgined) and usual out of RAM error
 *	
 *	Revision 2.15  2002/05/06 21:33:56  lewis
 *	<=============================== Led 3.0.1 Released ==============================>
 *	
 *	Revision 2.14  2001/11/27 00:30:02  lewis
 *	<=============== Led 3.0 Released ===============>
 *	
 *	Revision 2.13  2001/09/26 15:41:22  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.12  2001/08/28 18:43:43  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.11  2001/07/31 15:09:23  lewis
 *	use &* trick in front of iterator to get underlying array address (needed for
 *	MSVC70B2 STL - but should work for any STL)
 *	
 *	Revision 2.10  2000/06/12 20:06:48  lewis
 *	SPR#0780- new Led_Tablet_ stuff - supported on Mac
 *	
 *	Revision 2.9  2000/04/24 21:11:50  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.8  2000/04/14 22:40:28  lewis
 *	SPR#0740- namespace support
 *	
 *	Revision 2.7  2000/03/29 22:27:26  lewis
 ...
 *	
 *	Revision 2.6  2000/03/28 04:11:34  lewis
 *	fix SynaxColoring code to reacto to changes for SPR#0718
 *	
 *	Revision 2.5  2000/03/16 00:33:21  lewis
 *	WRONG COMMENT ON LAST CHECKIN - really just add include of stdafx.h
 *	
 *	Revision 2.4  2000/03/16 00:32:32  lewis
 *	Very preliminary first cut at this new module - part of SPR#0717
 *	
 *	Revision 2.3  1999/12/25 04:14:28  lewis
 *	in DTOR - also must do a REFRESH - cuz were deleting the stylemarkers
 *	
 *	Revision 2.2  1999/12/25 00:07:09  lewis
 ...
 *
 *	Revision 2.3  1999/07/19 19:24:27  lewis
 *	Lose obsolete qLedFirstIndex, use STL vector instead of Led_Array, and undo commenting out
 *	of refresh call (whcih caused display glitch and comment as to why)
 *	
 *	Revision 2.2  1997/07/23 23:40:19  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.1  1997/03/23  01:22:23  lewis
 *	*** empty log message ***
 *
...
{
	Led_RequireNotNil (ts);
	Led_Require (lookBackStart <= lookBackTo);
...
TableDrivenKeywordSyntaxAnalyzer::KeywordTable	TableDrivenKeywordSyntaxAnalyzer::kCPlusPlusKeywords (kCPlusPlusKeywordTable, Led_NEltsOf (kCPlusPlusKeywordTable));
...
TableDrivenKeywordSyntaxAnalyzer::KeywordTable	TableDrivenKeywordSyntaxAnalyzer::kVisualBasicKeywords (kVisualBasicKeywordTable, Led_NEltsOf (kVisualBasicKeywordTable), Led_tStrniCmp);
...
{
	Led_RequireNotNil (ts);
	Led_RequireNotNil (lookBackStart);
	Led_RequireNotNil (lookBackTo);
	Led_Require (*lookBackStart <= *lookBackTo);
	...
	*lookBackTo = min (ts->GetEnd (), *lookBackTo + fKeywordTable.MaxKeywordLength ()-1);
	Led_Ensure (*lookBackStart <= *lookBackTo);
}
...
void	SimpleSyntaxColoringMarkerOwner::DidUpdateText (const UpdateInfo& updateInfo) throw ()
{
...
{
	Led_Require (updateFrom <= updateTo);
	...
	Led_Assert (lookBackStart <= lookBackTo);
...
void	WindowedSyntaxColoringMarkerOwner::DidUpdateText (const UpdateInfo& updateInfo) throw ()
{
	inherited::DidUpdateText (updateInfo);
	if (fDeletedLines) {
		fDeletedLines = false;
