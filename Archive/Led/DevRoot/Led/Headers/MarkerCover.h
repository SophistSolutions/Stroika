...
/*
 * $Header: /cygdrive/k/CVSRoot/Led/Headers/MarkerCover.h,v 2.48 2003/12/08 02:35:03 lewis Exp $
 */
 ...
 * Changes:
 *	$Log: MarkerCover.h,v $
 *	Revision 2.48  2003/12/08 02:35:03  lewis
 *	use typename in a couple places to cleanup warnings from GCC 3.2
 *	
 *	Revision 2.47  2003/04/30 14:54:39  lewis
 *	tweeks to get compiling nicely with GCC 3.2 (RedHat 8.0)
 *	
 *	Revision 2.46  2003/04/18 17:01:34  lewis
 *	small code cleanups to things complained about by new gcc 2.96
 *	
 *	Revision 2.45  2003/03/27 15:47:39  lewis
 *	Deleted ifdefed code. SPR#1384: react to UpdateInfo and SimpleUpdater changes for realContentUpdate
 *	
 *	Revision 2.44  2003/03/16 16:47:57  lewis
 *	SPR#1345 - use SimpleUpdater instead of direct DoDidUpdateCalls() call. A bit tricky for case of MarkerCover<> usage!
 *	
 *	Revision 2.43  2003/01/29 19:15:08  lewis
 *	SPR#1265- use the keyword typename instead of class in template declarations
 *	
 *	Revision 2.42  2003/01/11 19:28:35  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.41  2002/10/07 23:20:30  lewis
 *	SPR#1118. make MarkerCover be a VIRTUAL MarkerOwner so we can have abstract StyleDatabase
 *	class and abstract paragraphdatabase etc.
 *	
 *	Revision 2.40  2002/05/06 21:33:31  lewis
 *	<=============================== Led 3.0.1 Released ==============================>
 *	
 *	Revision 2.39  2001/11/27 00:29:43  lewis
 *	<=============== Led 3.0 Released ===============>
 *	
 *	Revision 2.38  2001/11/08 20:04:11  lewis
 *	added helper CullZerod (size_t around) to help fix SPR#1076
 *	
 *	Revision 2.37  2001/11/08 16:37:51  lewis
 *	SPR#1072- make SetInfoInnerLoop () callable with NULL for last 2 args to facilitate fix for SPR#1072
 *	
 *	Revision 2.36  2001/10/20 13:38:56  lewis
 *	tons of DocComment changes
 *	
 *	Revision 2.35  2001/10/17 20:42:51  lewis
 *	Massive changes - cleaning up doccomments (mostly adding <p>/</p> as appropriate
 *	
 *	Revision 2.34  2001/09/18 20:16:34  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.33  2001/09/18 20:10:53  lewis
 *	SPR#1029- Speed tweek - added MarkerOfATypeMarkerSink<MARKER> template and used that in
 *	MarkerCover<>::GetInfo instead of CollectAllInRange () to avoid messing with vector CTOR/DTOR
 *	and copying. Came from MacOS profiling, but should help PC as well
 *	
 *	Revision 2.32  2001/09/05 00:18:10  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.31  2001/09/03 19:14:30  lewis
 *	SPR#0660,SPR#0762- rewrite much of the MarkerCover<> code to be simpler, clearer, and faster
 *	
 *	Revision 2.30  2001/08/29 23:00:17  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.29  2001/08/28 18:43:28  lewis
 *	*** empty log message ***
 *	
 *	Revision 2.28  2001/08/23 16:04:49  lewis
 *	SPR#0969- remove hack from MarkerCover<>::SetInfo () (end of buffer) (was for SPR#0559)
 *	
 *	Revision 2.27  2000/07/26 05:46:35  lewis
 ...
 *	
 *	Revision 2.26  2000/05/31 15:42:28  lewis
 *	kill ifdefed code
 *	
 *	Revision 2.25  2000/04/15 14:32:34  lewis
 *	small changes to get qLedUsesNamespaces (SPR#0740) working on Mac
 *	
 *	Revision 2.24  2000/04/14 22:40:21  lewis
 *	SPR#0740- namespace support
 *	
 *	Revision 2.23  2000/03/17 22:31:31  lewis
 ...
 *	
 *	Revision 2.22  1999/12/15 01:09:28  lewis
 *	(PrevComment wrong-accidnetal checkin)- Work around SPR#0659- Added  fNeedExtraUpdateCheck_UpdateInfo,
 *	and use that with fNeedExtraUpdateCheck. Track cleanup of this hack in SPR#0660
 *	
 *	Revision 2.21  1999/12/15 01:05:42  lewis
 *	Slight speed tweek in TextInteractor::UpdateIfNoKeysPending () - SPR#0659
 *	
 *	Revision 2.20  1999/12/14 18:03:02  lewis
 *	Only call invariant if qHeavyDebugging
 *	
 *	Revision 2.19  1999/11/13 16:32:18  lewis
 *	<===== Led 2.3 Released =====>
 *	
 *	Revision 2.18  1999/07/12 21:55:30  lewis
 *	use 'typename' keyword in template usage to avoid new warnings in MWERKS CW5 Pro compiler
 *	(MSVC60 seems happy either way)
 *	
 *	Revision 2.17  1999/06/29 01:48:52  lewis
 *	several subtle changes to the MarkerCover<> NeedExtraUpdateCheckMode caching code -
 *	see spr#0599 for details
 *	
 *	Revision 2.16  1999/06/28 19:42:46  lewis
 *	spr#0599- keep boolean fNeedExtraUpdateCheck so we can tell when an AboutToUpdate call
 *	has hapened, but no corresponding DidUpdateText call- and in that case - we agressively
 *	recheck marker constraints. This is needed because the AboutToUpdate/DidUpdate mechanism
 *	in TextStore for MarkerOwners doens't make good/useful gaurantees about he order in which
 *	multiple MarkerOwners will get DidUpdate calls. And some other marker owners might get done
 *	BEFORE the MarkerCover<> and yet call into the MarkerCover for info (a GetInfo() call).
 *	
 *	Revision 2.15  1999/05/03 22:04:56  lewis
 *	Misc cosmetic cleanups
 *	
 *	Revision 2.14  1999/03/25 22:26:35  lewis
 *	Add GetInfo/SetInfos () methods to take vector <pair <MARKERINFO,size_t(LENGTH)> - simple wrappers
 *	
 *	Revision 2.13  1999/03/12 17:29:53  lewis
 *	fix spr#0559
 *	
 *	Revision 2.12  1999/02/21 21:51:21  lewis
 *	add second arg to CTOR (defaulting to old emptyCTOR value). So we can provide externally
 *	default value, and not need to call SetInfo, which does a bunch of undesirbale (though not
 *	WRONG) DidUpdates
 *	
 *	Revision 2.11  1998/10/30 14:13:15  lewis
 *	Use vector<> instead of Led_Array<>
 *	
 *	Revision 2.10  1998/07/24  00:59:08  lewis
 *	Fix for UNICODE
 *
 *	Revision 2.9  1998/05/05  00:29:36  lewis
 *	support MWERKSCWPro3 - and lose old bug workarounds.
 *
 *	Revision 2.8  1998/04/25  01:22:02  lewis
 *	GetInfo() now returns const reference.
 *
 *	Revision 2.7  1998/04/09  01:26:33  lewis
 *	Use new MarkerMortuary<> template to delete items more directly, yet delayed.
 *	Added CollectAllNonEmptyInRange_OrSurroundings ()
 *	other minor cleanups/fixes
 *
 *	Revision 2.6  1998/04/08  01:12:53  lewis
 *	Alot of work to support ParagraphInfo covers (and forced alignment).
 *	Still maybe not quite right. But at least came up with decent (subclass/virtual methods)
 *	way to handle
 *	different needs of stylemarkers and paragraph marker needs.
 *
 *	Revision 2.5  1998/03/04  20:17:49  lewis
 *	*** empty log message ***
 *
 *	Revision 2.4  1997/12/24  04:36:55  lewis
 *	*** empty log message ***
 *
 *	Revision 2.3  1997/12/24  03:25:14  lewis
 *	Add first cut at DidUpdateText_CheckMarkerBounaryConstraints - but inadequate...
 *
 *	Revision 2.2  1997/10/01  01:55:31  lewis
 *	Only do the DIdUpdate_CheckForMerges and DIdUpdateText_CallZeroed() when the
 *	updateInfo.fTextModified flag is set
 *	true, or if we've done a SetInfo () call (sets flag).
 *
 *	Revision 2.1  1997/09/29  21:28:23  lewis
 *	Added CollectAllNonEmptyInRange () utility, and revised SetInfo() code to use it (in one place)
 *	to workaround bug introduced by change in TextStore::Overlap () definition.
 *
 *	Revision 2.0  1997/09/29  14:26:48  lewis
 *	*** empty log message ***
 *
 *
 *	<========== CODE based on StyledTextImager.h / StyleDatabase ==========>
 *
 *
 */
	...
	class	MarkerCover : public virtual MarkerOwner {
		...
		protected:
			class	NonEmptyOnes : public MarkersOfATypeMarkerSink2Vector<MARKER> {
				...
				public:
					override	void	Append (Marker* m)		{if (m->GetLength () != 0) {inherited::Append (m);}}
			};
...
//	class	MarkerCover<MARKER,MARKERINFO,INCREMENTALMARKERINFO>
	template	<typename	MARKER, typename	MARKERINFO, typename	INCREMENTALMARKERINFO>
		MarkerCover<MARKER,MARKERINFO,INCREMENTALMARKERINFO>::MarkerCover (TextStore& useTextStore, MARKERINFO defaultInfo):
				...
				fNeedExtraUpdateCheck_UpdateInfo (0, 0, NULL, 0, false, false)
			{
				/*
		...
		void	MarkerCover<MARKER,MARKERINFO,INCREMENTALMARKERINFO>::SetInfo (size_t charAfterPos, size_t nTCharsFollowing, const INCREMENTALMARKERINFO& infoForMarkers)
			{
				...
				TextStore::SimpleUpdater*	updater	=	NULL;
				try {
		...
		void	MarkerCover<MARKER,MARKERINFO,INCREMENTALMARKERINFO>::SetInfos2 (size_t charAfterPos, const vector<pair<MARKERINFO,size_t> >& infoForMarkers)
			{
				...
				for (typename vector<pair<MARKERINFO,size_t> >::const_iterator i = infoForMarkers.begin (); i != infoForMarkers.end (); ++i) {
					tmp.push_back (pair<MARKERINFO,size_t> (INCREMENTALMARKERINFO ((*i).first), (*i).second));
				}
	...
	template	<typename	MARKER, typename	MARKERINFO, typename	INCREMENTALMARKERINFO>
		...
		@DESCRIPTION:
			...
			outer scope, passed in variables that help the overall update process.</p>
				<p>Only update 'allMarkersInRange' - calling aboutToUpdate - if the passed in argument is non-NULL.
				Only update 'changedAnything' - flag if its pointer is non-NULL. 'allMarkersInRange' can be null if-and-only-if
		...
		*/
		void	MarkerCover<MARKER,MARKERINFO,INCREMENTALMARKERINFO>::SetInfoInnerLoop (
							...
							)
			{
				...
				MARKER*				prevNonEmptyMarker	=	NULL;	// used for coalescing...
				sort (markers.begin (), markers.end (), LessThan<MARKER> ());
				...
				for (typename MarkerVector::const_iterator mi = miStart; mi != miEnd; mi++) {
					...
					if (fsp != m->GetInfo ()) {
						if (updater != NULL and *updater == NULL) {
