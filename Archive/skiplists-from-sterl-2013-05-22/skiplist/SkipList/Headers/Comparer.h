...
/*
 Sorted containers place requirments on the contained elements to allow ordering. The interface used here mimics cmpstr, because that 
...
template <>
struct Comparer<unsigned short> {
...
template <>
struct Comparer<unsigned long> {
	typedef	unsigned long	VType;
	static	long	Compare (VType v1, VType v2)
	{
		return (v1 -v2);
	}
};
...
struct Comparer<std::string> {
	...
	{
		return v1.compare (v2);	// not sure if this is case sensitive
		//return (_stricmp (v1.c_str (), v2.c_str ()));
	}
};