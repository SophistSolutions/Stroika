...
regression-test-configurations:
	...
	if [ `uname -o` = "Cygwin" ] ; then\
		...
		#So far cannot figure how to (generically) cross-compile OpenSSL\
		./configure raspberrypi-gcc-4.9 --assertions enable --trace2file enable --compiler-driver 'arm-linux-gnueabihf-g++-4.9' --cross-compiling true --OpenSSL no;\
	fi
