...
basic-unix-test-configurations:
	...
	./configure g++-release-sanitize_thread_undefined --config-tag Unix --apply-default-release-flags --trace2file enable --cppstd-version c++17 --sanitize none,thread,undefined --append-run-prefix 'TSAN_OPTIONS=suppressions=ThreadSanitizerSuppressions-Ubuntu1810-x86_64.supp';\
	...
	./configure g++-valgrind-release-SSLPurify-NoBlockAlloc --config-tag Unix --config-tag valgrind --valgrind enable --openssl use --openssl-extraargs purify  --apply-default-release-flags --trace2file disable --block-allocation disable;
