...
regression-test-configurations:
	...
	if [ "$(UNAME_DASH_O_)" = "Cygwin" ] ; then\
		...
		#./configure my-clang-3.9.1-libc++-debug --compiler-driver ~/clang-3.9.1/bin/clang++ --append-run-prefix 'LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:~/clang-3.9.1/lib' --apply-default-debug-flags --only-if-has-compiler --static-link-gccruntime disable --cppstd-version-flag --std=c++14 --append-extra-compiler-and-linker-args -stdlib=libc++ --append-extra-linker -lc++experimental --sanitize none;\
		./configure my-clang-4.0.1-libc++-debug --compiler-driver ~/clang-4.0.1/bin/clang++ --append-run-prefix 'LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:~/clang-4.0.1/lib' --apply-default-debug-flags --only-if-has-compiler --static-link-gccruntime disable --cppstd-version-flag --std=c++14 --append-extra-compiler-and-linker-args -stdlib=libc++ --append-extra-linker-args -lc++experimental --sanitize none;\
		./configure my-clang-5.0.0-libc++-debug --compiler-driver ~/clang-5.0.0/bin/clang++ --append-run-prefix 'LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:~/clang-5.0.0/lib' --apply-default-debug-flags --only-if-has-compiler --static-link-gccruntime disable --append-extra-compiler-and-linker-args -stdlib=libc++ --append-extra-linker-args -lc++experimental --sanitize none;\
