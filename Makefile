...
regression-test-configurations:
	...
	if [ `uname -o` = "Cygwin" ] ; then\
		...
		###TMPDISABLE./configure my-clang-5.0.0-libc++-debug --compiler-driver ~/clang-5.0.0/bin/clang++ --apply-default-debug-flags --only-if-has-compiler --static-link-gccruntime disable --append-extra-compiler-and-linker-args -stdlib=libc++ --sanitize none;\
