...
regression-test-configurations:
	...
	if [ "$(UNAME_DASH_O_)" = "Cygwin" ] ; then\
		...
		./configure my-g++-8.2-debug-c++17 --compiler-driver ~/gcc-8.2.0/bin/x86_64-pc-linux-gnu-gcc --apply-default-debug-flags --no-sanitize address --append-run-prefix 'LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:~/gcc-8.2.0/lib64' --only-if-has-compiler --cppstd-version c++17;\
		#\
