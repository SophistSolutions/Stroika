...
libraries:
	...
	@if [[ -f Builds/$(CONFIGURATION)/Stroika-Foundation${LIB_SUFFIX} && -f Builds/$(CONFIGURATION)/Stroika-Frameworks${LIB_SUFFIX} ]]; then \
		...
		$(MAKE) --no-print-directory --silent .vscode/c_cpp_properties.json IntermediateFiles/$(CONFIGURATION)/Stroika-Current-Version.h;\
...
check-prerequisite-tools-common:
	...
	@ScriptsLib/PrintProgressLine $$(($(MAKE_INDENT_LEVEL)+1)) -n && sh -c "(type python3 2> /dev/null) || (ScriptsLib/GetMessageForMissingTool python3 && exit 1)"
	...
	@ScriptsLib/PrintProgressLine $$(($(MAKE_INDENT_LEVEL)+1)) -n && sh -c "(type xxd 2> /dev/null) || (ScriptsLib/GetMessageForMissingTool xxd && exit 1)"
...
ifneq ($(CONFIGURATION),)
	@$(StroikaRoot)ScriptsLib/CheckValidConfiguration $(CONFIGURATION)
	@$(MAKE) --no-print-directory --silent IntermediateFiles/$(CONFIGURATION)/Configuration.mk .vscode/c_cpp_properties.json IntermediateFiles/$(CONFIGURATION)/Stroika-Current-Version.h
endif
