...
sub GetAugmentedEnvironmentVariablesForConfiguration
{
	...
	if ($ARCH eq "x86") {
		...
		if ($nEXEDirs > 1) {
			print(`Build/ScriptsLib/PrintLevelLeader \$(($MAKE_INDENT_LEVEL+2))` . "Configure Warning: Multiple Compiler directories found: using $exe32Dir\n");
		}
		...
		if ($nEXEDirs > 1) {
			print(`Build/ScriptsLib/PrintLevelLeader \$(($MAKE_INDENT_LEVEL+2))` . "Configure Warning: Multiple Compiler directories found: using $exe64Dir\n");
		}
